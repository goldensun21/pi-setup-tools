...
sed -i '' "1,/ROOTEP/{s/set ROOTEP.*/set ROOTEP $ROOTP/;}" ./config_scripts/expsetup.exp

#get new desired username and pass to expsetup.exp and setup.sh
echo
echo "Please enter a new username for the pi"
read NEWU
sed -i '' "s/NEWEU/$NEWU/" ./config_scripts/setup.sh
sed -i '' "s/set NEWEU.*/set NEWEU $NEWU/" ./config_scripts/expsetup.exp

#get new desired password and pass to expsetup.exp
echo
echo "Please enter a new password for this account"
read -s NEWP
sed -i '' "s/set NEWEP.*/set NEWEP $NEWP/" ./config_scripts/expsetup.exp
...
then
        sed -i '' "s/NEWHOST/raspberrypi/" ./config_scripts/setup.sh
else
        sed -i '' "s/NEWHOST/$NEWHOSTNAME/" ./config_scripts/setup.sh
fi
...
echo
sleep 2
sed -i '' "s/set IPADDR.*/set IPADDR $PIIP/" ./config_scripts/expect.exp
expect ./config_scripts/expect.exp


#cleanup

sed -i '' "s/set ROOTEP.*/set ROOTEP xxx/" ./config_scripts/expsetup.exp
sed -i '' "s/set NEWEP.*/set NEWEP xxx/" ./config_scripts/expsetup.exp
sed -i '' "s/set NEWEU.*/set NEWEU xxx/" ./config_scripts/expsetup.exp
sed -i '' "s/set IPADDR.*/set IPADDR xxx/" ./config_scripts/expect.exp
sed -i '' "s/$NEWU/NEWEU/" ./config_scripts/setup.sh
sed -i '' "s/$NEWHOSTNAME/NEWHOST/" ./config_scripts/setup.sh
sed -i '' "s/raspberrypi/NEWHOST/2" ./config_scripts/setup.sh
sed -i '' "s/127.0.1.1     $NEWHOSTNAME/127.0.1.1     NEWHOST/" ./config_scripts/setup.sh
sed -i '' "s/127.0.1.1     raspberrypi/127.0.1.1     NEWHOST/" ./config_scripts/setup.sh
